...
| AGENT-003 | Done | PR 3 (revamp) | frontend | Wire UI to live event stream | `src/simulator/useAgentRun.ts` subscribes to `/api/events`, dispatches `WireEvent`s through a reducer, projects per-scenario state. Six tabs read from live arrays (timeline, shell, rag, policy, audit) with fixture fallback when no run has happened yet. New affordances: validator-override banner in Policy view, sandbox/dev mode pill in scenario header, bridge-offline banner. Replay-run button POSTs `/api/run`. Old `useScenarioRunner` retired. Closes LIVE-005. |
| AGENT-004 | Done | PR 4 (revamp) | live-nemoclaw | Latency timeout + graceful fallback | Three layers of timeouts: per-tool spawn (`OPENSHELL_TOOL_TIMEOUT_MS`, default 8s), Nemotron fetch (`NEMOTRON_TIMEOUT_MS`, default 30s, AbortController-based), per-run watchdog (`RUN_TIMEOUT_MS`, default 90s). Agent loop catches `NemotronTimeoutError` and emits a deterministic `policy.verdict` from the validator. Bridge watchdog kills stalled runners and synthesizes any missing events (rules, verdict, audit, recommendation, run.complete) so the demo always reaches a verdict. 31 tests pass. |
