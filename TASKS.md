...
| AGENT-002b | Backlog | PR 2b (revamp) | live-nemoclaw | Nemotron tool-calling loop via OpenShell inference proxy | Replace scripted dispatch with real Nemotron tool-calling against the `inference.local` endpoint. Validator from PR 1 retains veto power. Falls back to scripted mode when inference route isn't configured. |
