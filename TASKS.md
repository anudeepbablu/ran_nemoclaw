...
| AGENT-001 | Backlog | PR 1 (revamp) | policy-engine | Extract deterministic Policy Engine validator | Pure TS module that derives `Decision` from `PolicyRule[]`. Validator owns final verdict and can override an LLM proposal. Tests assert validator output matches each scenario's hardcoded decision. |
| AGENT-002 | Backlog | PR 2 (revamp) | live-nemoclaw | Bridge: Nemotron tool-calling + OpenShell-routed tools | Bridge calls Nemotron via build.nvidia.com REST. Tool surface: `inspectConfigChange`, `runValidationTests`, `queryInventory`, `fetchVendorSchema`, `applyChange` (all OpenShell), plus `searchRagCorpus`, `proposePolicyDecision`, `recordAudit`, `finalizeRecommendation` (pure TS). Streams `AgentEvent`s shaped to the PR 0 contract. |
